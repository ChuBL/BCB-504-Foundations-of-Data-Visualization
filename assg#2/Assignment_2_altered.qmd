...
The 'id' field is in a strict ascending order, while not continuous. The 'id' field of each row is determined by the website managers or data providers, therefore it has nothing to do with some standard identifications.
The 'names' field indicates the IMA approved mineral species names.
The 'elements' and 'sigelements' fields indicating the elements of the mineral chemical formual, while the 'sigelement' is determined by some significant elements as a subset of 'elements'. In compatable with csv format, the elements in this field are separated by hyphens $-$.
